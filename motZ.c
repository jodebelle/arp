...
static void signal_handler(int sig) {
    if (sig == SIGUSR1) { // Command console 
        ...
        // Exit
        if (command_c[0] == 'q') {
            write(fd_IZ, command_c, strlen(command_c)+1); 
            close(fd_IZ);
            exit(EXIT_SUCCESS);
        }
        ...
        write(fd_IZ, position, strlen(position)+1);
        printf("Speed: %i\n", speed); fflush(stdout); 
...
int main() {
    ...
    mkfifo(myfifo_zi, (int)0666);
    mkfifo(myfifo_z, (int)0666);
    mkfifo(myfifo_IZ, (int)0666);
    /*
